...
static void print_help(const char* prog) {
    std::cout
        << "Usage:\n"
        << "  " << prog << " RUNFILE [INPUT_ROOT] [OUTPUT_ROOT]\n"
        << "  " << prog << " -h | --help\n\n"
        << "Sort NestDAQ FileSink raw data split by auto sub-channel into three\n"
        << "output files grouped by femId (ascending). STF byte contents are copied\n"
        << "unchanged. HBF number continuity and input/output total size are checked.\n\n"
        << "Arguments:\n"
        << "  RUNFILE      Raw-data filename, e.g. run000020.dat\n"
        << "  INPUT_ROOT   Input root directory  (default: rawdata)\n"
        << "  OUTPUT_ROOT  Output root directory (default: rawdata_sorted)\n\n"
        << "Default layout:\n"
...
int main(int argc, char** argv) {
    ...
    std::array<Input,3> in;
    uint64_t input_total=0;
    for (size_t i=0;i<3;++i) {
        std::ostringstream d; d<<std::setw(2)<<std::setfill('0')<<i;
        in[i].path=inroot/d.str()/runfile;
    ...
    for(size_t fi=0;fi<3;++fi) {
        ...
        x.header.resize(sizeof(FileHeader)); x.f.clear(); x.f.seekg(0);
        ...
        while(true) {
            ...
            if(magic==STF_MAGIC) {
                ...
                Record rec{sh.timeFrameId,sh.femId,sh.length,uint16_t(fi),uint64_t(pos),serial++};
                femids.insert(sh.femId); stf_bytes+=sh.length;

                // Inspect all payload words exactly as nestdaq-rawdata-tools/hbf_num_checker does.
                uint64_t payload=sh.length-sizeof(sh);
                uint64_t nword=payload/8;
                uint32_t prev_hb=0;
                for(uint64_t w=0;w<nword;++w){
                    uint64_t word=0;
                    if(!read_exact(x.f,&word,8)){ std::cerr<<"\nERROR: truncated STF payload\n"; return 7; }
                ...
                if(rem) x.f.seekg(static_cast<std::streamoff>(rem),std::ios::cur);
                scan_done += sh.length;
                scanprog.update(scan_done);
                x.f.clear(); x.f.seekg(static_cast<std::streamoff>(end));
            } else if(magic==TRL_MAGIC) {
                uint64_t tpos=uint64_t(pos), tlen=x.size-tpos;
                ...
                scan_done += tlen; scanprog.update(scan_done);
                break;
            } else {
    ...
    if(femids.size()!=3){
        std::cerr<<"ERROR: expected exactly 3 femIds, found "<<femids.size()<<"\n"; return 11;
    }
    std::vector<uint32_t> order(femids.begin(),femids.end());
    std::map<uint32_t,size_t> outidx; for(size_t i=0;i<3;++i) outidx[order[i]]=i;

    std::stable_sort(records.begin(),records.end(),[](const Record&a,const Record&b){
        if(a.femid!=b.femid) return a.femid<b.femid;
        if(a.tfid!=b.tfid) return a.tfid<b.tfid;
        return a.serial<b.serial;
    });

    // Check HBF continuity in the SAME order that will be written to the outputs.
    std::map<uint32_t,HbfStats> hbf;
    for (const auto& r : records) {
        auto& s = hbf[r.femid];
        if (!r.hb_seen) continue;
        if (!s.seen) {
            s.seen=true;
            s.first=r.first_hb;
        } else {
            ...
            if (r.first_hb!=expected) {
                ++s.discontinuities;
                if(s.first_gaps.size()<10) s.first_gaps.emplace_back(s.last,r.first_hb);
            }
        }
    ...
    for(size_t i=0;i<3;++i){
        ...
        std::cout<<"  ["<<i<<"] femId="<<hex32(id)<<" ("<<ipv4(id)<<")"
                 ...
                 <<"  count="<<s.count<<"  discontinuities="<<s.discontinuities<<"\n";
        for(auto &g:s.first_gaps) std::cout<<"       gap: "<<g.first<<" -> "<<g.second<<"\n";
    }
    bool same_start=true; uint32_t start_hb=hbf[order[0]].first;
    for(auto id:order) same_start &= hbf[id].seen && hbf[id].first==start_hb;
    std::cout<<"  start alignment: "<<(same_start?"OK (all equal)":"WARNING (different)")<<"\n";
    uint32_t minlast=0xffffff, maxlast=0;
    for(auto id:order){minlast=std::min(minlast,hbf[id].last);maxlast=std::max(maxlast,hbf[id].last);}
    std::cout<<"  stop spread    : "<<(maxlast-minlast)<<" HBF (simple non-wrap comparison)\n";

    std::array<std::ofstream,3> out;
    std::array<fs::path,3> outpath;
    std::array<uint64_t,3> outbytes{0,0,0}, outstf{0,0,0};
    for(size_t i=0;i<3;++i){
        std::ostringstream d; d<<std::setw(2)<<std::setfill('0')<<i;
    ...
    uint64_t copy_total=stf_bytes, copy_done=0;
    Progress copyprog("Copy sorted STF   ",copy_total);
    // Large reusable buffer; small STFs fit in one read, huge STFs are chunked.
    constexpr size_t BUFSZ=4*1024*1024;
    std::vector<char> buf(BUFSZ);

    for(const auto&r:records){
        size_t oi=outidx[r.femid]; auto& src=in[r.input_index].f;
        src.clear(); src.seekg(static_cast<std::streamoff>(r.offset));
    ...
    for(size_t i=0;i<3;++i){
        ...
        outbytes[i]+=in[i].trailer.size(); out[i].close();
    }
    ...
    for(size_t i=0;i<3;++i){
        ...
        std::cout<<"  "<<outpath[i]<<"  femId="<<hex32(order[i])<<" ("<<ipv4(order[i])<<")"
